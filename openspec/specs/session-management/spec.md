...
- **WHEN** a signed-in user visits the home page and sessions they created exist in the database
...
- **WHEN** a signed-in user visits the home page and no sessions exist in the database for that user
...
> **Tests:** [`e2e/home.spec.ts`](../../../e2e/home.spec.ts)
- **WHEN** a user navigates directly to `/sessions/{id}` for a session that exists
...
> **Tests:** none
- **WHEN** a user navigates to `/sessions/{id}` for an id that does not exist in the database
...
> **Tests:** [`e2e/home.spec.ts`](../../../e2e/home.spec.ts)
- **WHEN** a user on a session page clicks "Copy invite link"
