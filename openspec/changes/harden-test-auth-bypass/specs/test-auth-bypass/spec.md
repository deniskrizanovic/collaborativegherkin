...
### Requirement: The bypass authenticates only with the correct server-side secret

The `authorize` callback of the `test-bypass` provider SHALL sign a caller in only
when the submitted credential matches the server-side `TEST_AUTH_SECRET`. A
submission whose secret does not match MUST NOT create a session, and MUST NOT
